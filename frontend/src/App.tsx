...
const App: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/history")
      .then((res) => res.json())
      .then((data) => {  
        data.sort((a: { timestamp: string }, b: { timestamp: string }) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        setHistory(data); 
  ...
  return (
    <div className={`${isDarkMode ? "bg-[#191A1A] text-white" : "bg-[#F1F1F1] text-[#1E3D43]"} min-h-screen flex`}>
            ...
            <ul className="mt-5">
              {history.map((history_query, index) => (
                ...
                onClick={() => {
                  ...
                  key={index}
                  className={`flex justify-between items-center px-4 py-3 ${isDarkMode ? (isHistorySelected[index] ? "hover:bg-gray-700 bg-gray-700" : "hover:bg-gray-700")  : (isHistorySelected[index] ? "hover:bg-gray-300 bg-gray-300" : "hover:bg-gray-300")} overflow-y-auto hover: cursor-pointer`}>
                  ...
                  */}
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="0.5" className="size-6 hover: cursor-pointer" stroke={isDarkMode ? "white" : "black"}>
      ...
      {/* Main */}
      <div className={`flex-1 flex flex-col items-center justify-start px-4 ${searched ? "mt-[5vh]" : "mt-[30vh]"}`}>
        
        {!searched && (<h1 className="text-2xl md:text-4xl mb-10">Not Deep Search</h1>)}
        {/* Search Bar */}
        <div className="relative">
          <div className={`flex items-center ${isDarkMode ? "bg-[#222222] text-white" : "bg-[#F1F1F1] text-[#1E3D43]"} px-4 py-2 h-[60px] w-[700px] rounded-tl-2xl rounded-tr-2xl ${suggestions.length > 0 && !isSuggestionClicked ? "" : "rounded-bl-2xl rounded-br-2xl"} border ${isDarkMode ? "border-[#343636]" : "border-[#C3C3C1]"} shadow hover: cursor-pointer`}>
            <svg 
          ...
          {suggestions.length > 0 && !isSuggestionClicked && (
            <ul className={` z-50  ${isDarkMode ? "bg-[#222222] text-white" : "bg-[#F1F1F1] text-[#1E3D43]"} rounded-bl-2xl rounded-br-2xl shadow overflow-y-auto max-h-60`}>
              {suggestions.map((suggestion, index) => (
                <li
                  key={index}
                  className={`px-4 py-2 cursor-pointer border ${isDarkMode ? "hover:bg-gray-600 border-[#343636]" : "hover:bg-gray-300 border-[#C3C3C1]"}`}
                  onClick={() => {
                    setQuery(suggestion);
                    setIsSuggestionClicked(true);
                  }}
          ...
          {searchResult.length > 0 && (
            <div className="relative w-full h-[85vh] overflow-y-auto">
              {searchResult.map((item, index) => (
                  ...
                  key={index}
                  >
                  <SearchResultCard {...item} isDarkMode={isDarkMode}/>
                  </div>
              ))}
            </div>
            )
